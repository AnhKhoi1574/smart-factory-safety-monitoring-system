...
With that setting, notebook `03_split_dataset.ipynb` should split `data/master_original/` into train and validation only. It must not reserve another 10% test split from `master_original`, because the final test set already comes from `data/test_sources/`.
